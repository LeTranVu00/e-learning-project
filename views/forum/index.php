<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8" 
    ...
    <!-- Header -->
    <div class="flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 mb-6 gap-4" data-aos="fade-up">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
                <i class="fa-solid fa-comments text-primary mr-2"></i> Cộng đồng học tập
            </h1>
            <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Hỏi đáp, chia sẻ kiến thức và thảo luận cùng mọi người.</p>
        </div>
        
        <div class="flex items-center gap-3 w-full sm:w-auto">
            <?php if(($_GET['filter'] ?? '') === 'my_posts'): ?>
                <a href="?action=forum" class="flex-1 sm:flex-none text-center bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 font-bold py-2.5 px-5 rounded-xl transition-all duration-300 shadow-sm hover:shadow-md">
                    <i class="fa-solid fa-globe mr-1"></i> Tất cả bài
                </a>
            <?php else: ?>
                <a href="?action=forum&filter=my_posts" class="flex-1 sm:flex-none text-center bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 text-blue-600 dark:text-blue-400 font-bold py-2.5 px-5 rounded-xl transition-all duration-300 shadow-sm border border-blue-100 dark:border-blue-800 hover:shadow-md">
                    <i class="fa-solid fa-user-pen mr-1"></i> Bài của tôi
                </a>
            <?php endif; ?>

            <button @click="showPostModal = true" class="flex-1 sm:flex-none bg-primary hover:bg-yellow-600 text-white font-bold py-2.5 px-5 rounded-xl transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 flex items-center justify-center gap-2">
        ...
        <form action="" method="GET" class="flex flex-col md:flex-row gap-3">
            ...
            <div class="w-full md:w-48 relative" x-data="{ 
                ...
                <button type="button" @click="open = !open" @click.away="open = false" 
                    ...
                    <i class="fa-solid fa-chevron-down text-gray-400 transition-transform duration-200 text-xs" :class="open ? 'rotate-180' : ''"></i>
                </button>
    ...
    <div class="space-y-4">
        <?php if(empty($posts)): ?>
            <div class="bg-white dark:bg-gray-800 p-12 rounded-2xl text-center border border-gray-100 dark:border-gray-700 shadow-sm" data-aos="fade-up">
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-2 flex-wrap">
                                <h3 class="font-bold text-gray-900 dark:text-white"><?= htmlspecialchars($post['author_name']) ?></h3>
                                <?php if($post['author_role'] == 'admin'): ?>
                                    <span class="text-[10px] bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 font-bold px-2 py-0.5 rounded-full uppercase">
                                    ...
                                    <?php if($_SESSION['user_id'] == $post['user_id'] || $_SESSION['user_role'] === 'admin'): ?>
                                        <div x-data="{ openMenu: false }" class="relative">
                                            ...
                                            <div x-show="openMenu" 
                                                ...
                                                <button @click="openEdit(<?= htmlspecialchars(json_encode($post), ENT_QUOTES, 'UTF-8') ?>); openMenu = false" 
    ...
    <div x-show="showPostModal" 
        ...
        <div class="fixed inset-0 bg-gray-900/60 dark:bg-black/70 backdrop-blur-sm transition-opacity" @click="showPostModal = false"></div>
    ...
    <div x-show="showEditModal" 
         ...
         x-cloak 
         class="fixed inset-0 z-50 overflow-y-auto">
        <div class="fixed inset-0 bg-gray-900/60 dark:bg-black/70 backdrop-blur-sm transition-opacity" @click="showEditModal = false"></div>
        <div class="flex items-center justify-center min-h-screen px-4 py-8">
            <div x-show="showEditModal" 
                 x-transition:enter="transition ease-out duration-300"
                 x-transition:enter-start="opacity-0 scale-95 translate-y-4"

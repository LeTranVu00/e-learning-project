...
<div class="max-w-4xl mx-auto" x-data="{ 
    ...
    <?php if (isset($_SESSION['success'])): ?>
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)"
            ...
            <div>
                <p class="text-sm text-gray-500 font-medium">Thành công</p>
                <p class="text-gray-800 font-bold"><?= htmlspecialchars($_SESSION['success']) ?></p>
            </div>
        </div>
    ...
    <?php if (isset($_SESSION['error'])): ?>
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)"
            ...
            x-transition:enter-end="opacity-100 translate-x-0" x-transition:leave="transition ease-in duration-300"
            x-transition:leave-start="opacity-100 translate-x-0" x-transition:leave-end="opacity-0 translate-x-full"
            class="fixed top-20 right-6 z-50 flex items-center bg-white border-l-4 border-red-500 rounded-xl shadow-xl p-4 min-w-[300px]">
            <div class="text-red-500 mr-3"><i class="fa-solid fa-circle-exclamation text-2xl"></i></div>
            <div>
                <p class="text-sm text-gray-500 font-medium">Lỗi</p>
                <p class="text-gray-800 font-bold"><?= htmlspecialchars($_SESSION['error']) ?></p>
            </div>
        </div>
    ...
    <?php if (empty($curriculum)): ?>
        <div
            class="text-center bg-white p-10 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div class="space-y-6">
            <?php foreach ($curriculum as $chapter): ?>
                <div
                    ...
                    <div
                        class="bg-gray-50 px-6 py-4 flex flex-col sm:flex-row sm:justify-between sm:items-center border-b border-gray-200 gap-3">
            ...
            <form action="?action=admin_store_chapter" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="title" required placeholder="VD: Chương 1 - Nhập môn"
                        class="w-full px-4 py-2 border border-gray-300 bg-white rounded-xl focus:ring-2 focus:ring-primary outline-none transition">
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-semibold mb-2">Mô tả chương (Tùy chọn)</label>
    ...
    <div x-show="showMaterialModal" x-cloak x-transition:enter="transition ease-out duration-300"
        ...
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl p-6 relative"
            ...
            <form action="?action=admin_store_material" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-semibold mb-2">Nội dung chi tiết / Mô tả (Tùy chọn)</label>
                    <textarea name="description" id="editor-add-material"></textarea>
                </div>

                <!-- Khu vực tải file -->
                <div x-show="materialTab === 'file'">
                    <label class="block text-sm font-semibold mb-2">Tải file lên (Mỗi file = 1 Bài giảng)</label>
                    ...
                    <div x-show="addFiles.length > 0" x-cloak class="mt-3 space-y-2 max-h-40 overflow-y-auto pr-2">
                        <template x-for="(file, index) in addFiles" :key="index">
                            <div class="flex items-center gap-2 text-sm text-gray-700 bg-gray-50 p-2 rounded-lg border border-gray-200">
                ...
                <div x-show="materialTab === 'link'">
                    <div class="flex items-center justify-between mb-2">
                        <label class="block text-sm font-semibold">Đường dẫn ngoài (Mỗi link = 1 Bài giảng)</label>
                            ...
                            <div class="flex items-center gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
                                <div class="flex-1 space-y-3">
                                    <input type="text" x-model="link.title" :name="'link_titles['+index+']'" placeholder="Tên bài giảng cho link này..."
                                        class="w-full px-3 py-2 text-sm border border-gray-300 bg-white rounded-lg outline-none focus:ring-2 focus:ring-primary transition">
                ...
                <div x-show="materialTab === 'quiz'">
                    <div class="mb-3">
                        <label class="block text-sm font-semibold mb-2">Tiêu đề bài Trắc nghiệm</label>
    ...
    <div x-show="showEditMaterialModal" x-cloak x-transition:enter="transition ease-out duration-300"
        ...
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl p-6 relative"
            ...
            <form action="?action=admin_update_material" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div wire:ignore>
                    <label class="block text-sm font-semibold mb-2">Nội dung chi tiết / Mô tả (Tùy
                        chọn)</label>
                ...
                <div x-show="editMaterialData.type === 'file'" x-collapse>
                    ...
                    <div x-show="editFileName" x-cloak class="mt-2 text-sm text-gray-600 flex items-center justify-center gap-2">
